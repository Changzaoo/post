    ...
    <Layout>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 18, flexWrap: 'wrap' }}>
          {categories.map(cat => (
            <button key={cat} onClick={() => setFilter(cat)} className={`btn btn-sm ${filter === cat ? 'btn-primary' : 'btn-secondary'}`}>
              {cat}
            </button>
          ))}
        </div>
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Category badge */}
                  <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 20, background: 'rgba(255,255,255,0.05)', color: 'var(--text-tertiary)', border: '0.5px solid var(--border)', flexShrink: 0 }}>
                    {item.category}
                  </span>
                </div>
              </motion.div>
